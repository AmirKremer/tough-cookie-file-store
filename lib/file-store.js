...
function FileCookieStore(clientFactory, cookieId = 'default') {
    Store.call(this);
    this.idx = {}; // idx is memory cache
    this.filePath = cookieId;
...
FileCookieStore.prototype.loadFromClient = function(cb) {
    var self = this;
    loadFromFile(this.clientFactory, this.filePath, function(dataJson) {
        if (dataJson) {
            self.idx = dataJson;
        }
        cb(dataJson);
...
FileCookieStore.prototype.putCookie = function(cookie, cb) {
    ...
    saveToFile(this.clientFactory, this.filePath, this.idx, function() {
        cb(null);
    });
};
...
var _writing_ = {};
function writeFile(client, filePath, data, done) {
    var dataJson = JSON.stringify(data);
    var wo = { done: done, next: [] };
    _writing_[filePath] = wo;
    client.set(filePath, dataJson, function (err) {
        // If we have new pending writes, execute them now
        if ( wo.next.length ) {
            writeFile(client, filePath, wo.data, wo.next);
        }
        else {
...
function saveToFile(clientFactory, filePath, data, cb) {
    var wo = _writing_[filePath];
    // If not writing to this file, start writing right now
    if ( !wo ) {
        writeFile(clientFactory(), filePath, data, [cb]);
    }
...
function loadFromFile(clientFactory, filePath, cb) {
    var client = clientFactory();
    client.get(filePath, function (err, data) {
      client.quit();
